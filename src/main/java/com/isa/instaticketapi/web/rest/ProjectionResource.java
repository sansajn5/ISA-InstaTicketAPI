...
public class ProjectionResource {
	 ...
	 *            object for editing
	 */
	@ApiOperation(value = "Edit projection", response = HttpStatus.class)
	@ApiResponses(value = { @ApiResponse(code = 201, message = "Edit projection"),
			@ApiResponse(code = 400, message = "Some attribute is already in use"),
			@ApiResponse(code = 401, message = "You are not authorized to view the resource"),
			@ApiResponse(code = 403, message = "Accessing the resource you were trying to reach is forbidden"),
			@ApiResponse(code = 404, message = "The resource you were trying to reach is not found"),
	...
	public void editProejction(@PathVariable("id") Long id, @RequestBody ProjectionDTO projectionDTO) {
		if (projectionRepository.findOneById(id) == null) {
			throw new IllegalArgumentException("Invalid id!");
		}
		projectionService.editProjection(id,projectionDTO);
	}
}
